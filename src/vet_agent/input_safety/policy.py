"""
文件：src/vet_agent/input_safety/policy.py
作用：封装基础输入安全 OPA 策略裁决客户端与本地测试裁决器。
范围：负责将输入安全候选提交给策略层，并将策略结果转换为 Agent 安全信号。
说明：生产路径应使用 OPA；本地裁决器仅供未配置外部策略的测试或开发场景显式使用。
"""
...
class InputSafetyPolicyClient(Protocol):
    """定义基础输入安全策略裁决客户端协议。
...
class OpaInputSafetyPolicyClient(InputSafetyPolicyClient):
    """使用 OPA Data API 执行输入安全策略裁决。
    ...
    async def decide(
        ...
        try:
            ...
            raise RuntimeError("input safety OPA policy decision failed") from exc
    ...
    def _unwrap_result(self, result: Any) -> dict[str, Any]:
        ...
        if isinstance(result, dict):
            return dict(result)
        raise RuntimeError("input safety OPA policy returned invalid decision payload")


class LocalInputSafetyPolicyClient(InputSafetyPolicyClient):
    """本地输入安全策略裁决器。

    说明：该实现只按照已结构化候选的严重级别做最小动作映射，不读取用户文本，不实现关键词规则。
    ...
    async def decide(
        ...
        del context
        if not candidates:
            return InputSafetyDecision.allow_turn(metadata={"policy_backend": "local"})
        if any(candidate.severity == "blocked" for candidate in candidates):
            signals = tuple(candidate.to_signal(severity="blocked") for candidate in candidates)
            return InputSafetyDecision(
                action=InputSafetyDecisionAction.BLOCK,
                allow=False,
                message=_message_from_candidates(candidates),
                reasons=tuple(candidate.message for candidate in candidates),
                candidates=candidates,
                signals=signals,
                metadata={"policy_backend": "local"},
            )
        if any(candidate.severity == "urgent" for candidate in candidates):
            signals = tuple(candidate.to_signal(severity="urgent") for candidate in candidates)
            return InputSafetyDecision(
                action=InputSafetyDecisionAction.ESCALATE,
                allow=True,
                message=_message_from_candidates(candidates),
                reasons=tuple(candidate.message for candidate in candidates),
                candidates=candidates,
        ...
        return InputSafetyDecision(
            action=InputSafetyDecisionAction.OBSERVE,
            allow=True,
            message="基础输入安全策略记录候选并允许继续执行。",
            reasons=tuple(candidate.message for candidate in candidates),
            candidates=candidates,
            signals=signals,
            metadata={"policy_backend": "local"},
...
class DisabledInputSafetyPolicyClient(InputSafetyPolicyClient):
    """显式禁用输入安全策略裁决的测试客户端。

    说明：该实现仅供测试或临时嵌入场景通过依赖注入使用，容器默认构造不会选择该实现。
    ...
    async def decide(
        ...
        """
        del context
        return InputSafetyDecision.allow_turn(
            metadata={
                "policy_backend": "disabled",
                "candidate_count": len(candidates),
                "decision_id": f"input_safety_disabled_{uuid4().hex}",
            }
        )
...
def _decision_from_payload(
    ...
    try:
        action = InputSafetyDecisionAction(raw_action)
    except ValueError as exc:
        raise RuntimeError(f"invalid input safety policy action: {raw_action}") from exc
    allow = bool(
        payload.get(
            "allow",
            action
            in {
                InputSafetyDecisionAction.ALLOW,
                InputSafetyDecisionAction.OBSERVE,
                InputSafetyDecisionAction.ESCALATE,
            },
        )
    )
...
def _signals_from_payload(
    ...
    if isinstance(raw_signals, list):
        ...
        for item in raw_signals:
            ...
            signals.append(
                SafetySignal(
                    code=str(item.get("code") or "INPUT_SAFETY_POLICY"),
...
def _severity_from_action(action: InputSafetyDecisionAction) -> str:
    """根据策略动作转换安全信号级别。

    :param action: 输入安全策略动作。
    :return: 返回安全信号级别。
    """
    if action == InputSafetyDecisionAction.BLOCK:
        return "blocked"
    if action == InputSafetyDecisionAction.ESCALATE:
        return "urgent"
    return "caution"


def _message_from_candidates(candidates: tuple[InputSafetyCandidate, ...]) -> str:
    """根据候选集合生成默认裁决说明。

    :param candidates: 输入安全候选集合。
    :return: 返回默认裁决说明。
    """
    messages = [candidate.message for candidate in candidates if candidate.message]
    if messages:
        return "；".join(messages)
    return "基础输入安全策略识别到需要审计的候选。"
