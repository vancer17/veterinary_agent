...
class MappingReviewAction(StrEnum):
    """Finite governance action for an ambiguous or unmapped mention."""

    ADD_ALIAS = "add_alias"
    ADD_CANONICAL = "add_canonical"
    KEEP_AMBIGUOUS = "keep_ambiguous"
    REJECT_MODEL_OUTPUT = "reject_model_output"
    UPDATE_SUBJECT_CONSTRAINT = "update_subject_constraint"
...
class SubjectBinding(BaseModel):
    ...
    resolution_status: Literal["resolved", "ambiguous", "missing"] = "resolved"
    subject_candidates: list[str] = Field(default_factory=list, max_length=8)
    confidence: float = Field(ge=0.0, le=1.0)
...
class UnmappedMention(BaseModel):
    """An explicit mention that cannot yet be mapped to a canonical concept."""

    model_config = ConfigDict(extra="forbid")

    mention_id: str = Field(min_length=1, max_length=96)
    segment_id: str = Field(min_length=1, max_length=64)
    source_text: str = Field(min_length=1, max_length=320)
...
class EvidenceAnalysisOutput(BaseModel):
    ...
    unmapped_mentions: list[UnmappedMention] = Field(
