...
class ExpectedUnmappedMention(BaseModel):
    """A golden explicit mention intentionally left outside the vocabulary."""

    model_config = ConfigDict(extra="forbid")

    segment_index: int = Field(ge=0, le=63)
    source_text: str = Field(min_length=1, max_length=320)
    mapping_status: Literal["not_found", "ambiguous", "type_mismatch"] = "not_found"
    candidate_canonical_ids: list[str] = Field(default_factory=list, max_length=12)
...
class ExpectedSegment(BaseModel):
    ...
    expected_evidence_count: int | None = Field(default=None, ge=0, le=32)
...
class ExpectedBehavior(BaseModel):
    ...
    expect_gate_block: bool = False
    expected_gate_ids: list[str] = Field(default_factory=list, max_length=8)
...
class ExpectedTurn(BaseModel):
    ...
    unmapped_mentions: list[ExpectedUnmappedMention] = Field(
...
class InputPreprocessingEvaluation:
            ...
            for attempt in range(1, max(1, repeat) + 1):
                self.reports.append(
                    await self._run_case(
                        ...
                        mode=mode,
                        attempt=attempt,
                        segmentation_variant=segmentation_variant,
                    )
                )
        return self.reports
    ...
    async def _run_case(
        ...
        *,
        ...
        branch_states: dict[str, dict[str, Any] | None] = {
            "baseline_empty_success": None,
            "answer_now_only": None,
        }
        ...
        for turn_index, expected_turn in enumerate(case.turns, start=1):
                ...
                try:
                    ...
                    result = await self.analyzer.analyze(
                        ...
                        segmentation_override=(
                            self._expected_segmentation(
                                expected_turn=expected_turn,
                                turn_context=turn_context,
                            )
                            if segmentation_variant == "expected"
                            else None
                        ),
                    )
                    model_error = None
            ...
            if turn_report["status"] not in {"passed", "gate_blocked_as_expected"}:
                case_passed = False
    ...
    async def _evaluate_result(
        ...
        *,
        ...
        branch_states: dict[str, dict[str, Any] | None] | None = None,
    ) -> dict[str, Any]:
        failures: list[str] = []
        blocking_gates = result.failed_blocking_gates()
        expected_gate_ids = set(expected_turn.expected_behavior.expected_gate_ids)
        actual_blocking_gate_ids = {gate.gate_id for gate in blocking_gates}
        gate_block_is_expected = (
            expected_turn.expected_behavior.expect_gate_block
            and bool(blocking_gates)
            and (
                not expected_gate_ids
                or bool(actual_blocking_gate_ids & expected_gate_ids)
            )
        )
        if blocking_gates:
            if gate_block_is_expected:
                pass
            else:
                failures.extend(
        ...
        if (
            expected_turn.expected_behavior.simulate
            and consultation is not None
            and not gate_block_is_expected
        ):
            ...
            decision_branches: dict[str, dict[str, Any]] = {}
            baseline_semantic = SemanticExtractionResult(
                facts=[],
                observations=[],
                intent=SemanticIntent(),
                strategy="litellm_response_format",
                confidence=0.95,
                source_text=expected_turn.user_text[:240],
            )
            answer_only_semantic = SemanticExtractionResult(
                facts=[],
                observations=[],
                intent=consultation.semantic_result.intent,
                strategy="litellm_response_format",
                confidence=consultation.semantic_result.confidence,
                source_text=expected_turn.user_text[:240],
            )
            branch_inputs = {
                "baseline_empty_success": baseline_semantic,
                "answer_now_only": answer_only_semantic,
            }
            if branch_states is not None:
                for branch_name, branch_semantic in branch_inputs.items():
                    branch_decision = await service.update(
                        branch_states.get(branch_name),
                        expected_turn.user_text,
                        PetContext(verified_profile=self.suite.pet_profile),
                        policy_context=ConsultationStatePolicyContext(
                            request_id=f"eval-{branch_name}-{uuid4().hex[:10]}",
                            trace_id=f"trace-{branch_name}-{uuid4().hex[:10]}",
                            user_id="eval-user",
                            pet_id="eval-pet",
                            session_id="eval-session",
                        ),
                        task_domain=case.task_domain,
                        semantic_result=branch_semantic,
                        max_questions=3,
                    )
                    decision_branches[branch_name] = {
                        "action": "answer" if branch_decision.ready else "ask",
                        "mode": branch_decision.answerability.get("mode"),
                        "blocking_slots": branch_decision.missing_slots,
                        "unknown_slot_count": len(
                            branch_decision.state.evidence_profile.get(
                                "unresolved_slots"
                            )
                            or []
                        ),
                        "state": branch_decision.state.to_dict(),
                    }
                    branch_states[branch_name] = branch_decision.state.to_dict()
        ...
        cross_agent_gap = (
            bool(result.evidence.observations)
        ...
        return {
            "status": (
                ...
                if gate_block_is_expected and not failures
                else "passed"
                if not failures
                else "failed"
            ),
            "failures": failures,
            "expected_semantics": sorted(expected_semantics),
            "actual_semantics": sorted(actual_semantics),
    ...
    def _ideal_result(
        ...
        *,
        ...
        segmentation = self._expected_segmentation(
            expected_turn=expected_turn,
            turn_context=turn_context,
        )
        evidence = self._expected_evidence(expected_turn)
    ...
    def _expected_segmentation(
        self,
        *,
        expected_turn: ExpectedTurn,
        turn_context: TurnContext,
    ) -> SegmentationOutput:
        references_by_segment: dict[int, set[str]] = {}
        candidates_by_segment: dict[int, set[str]] = {}
        for item in expected_turn.observations:
            references_by_segment.setdefault(item.segment_index, set()).add(
                item.subject_reference
            )
            candidates_by_segment.setdefault(item.segment_index, set()).update(
                item.subject_candidates
            )
        segments = [
            SegmentModel(
                ...
                expected_evidence_count=segment.expected_evidence_count,
                subject_reference=(
                    segment.subject_reference
                    if segment.subject_reference is not None
                    else (
                        "subject_ambiguous"
                        if references_by_segment.get(index - 1, set())
                        == {"subject_ambiguous"}
                        or len(references_by_segment.get(index - 1, set())) > 1
                        else next(
                            iter(references_by_segment.get(index - 1, set())), None
                        )
                    )
                ),
                subject_candidates=(
                    segment.subject_candidates
                    or sorted(
                        candidates_by_segment.get(index - 1, set())
                        or references_by_segment.get(index - 1, set())
                    )
                ),
                confidence=0.98,
            )
    ...
    def _expected_evidence(self, expected_turn: ExpectedTurn) -> EvidenceAnalysisOutput:
        observations = [
            EvidenceObservation(
        ...
        unmapped_mentions = [
            UnmappedMention(
                mention_id=f"unmapped-{index}",
                segment_id=f"seg-{item.segment_index + 1}",
                source_text=item.source_text,
                mapping_status=item.mapping_status,
                candidate_canonical_ids=item.candidate_canonical_ids,
                reason_code=item.reason_code,
                confidence=item.confidence,
            )
            for index, item in enumerate(expected_turn.unmapped_mentions, start=1)
        ]
    ...
    def _expected_subject(self, item: ExpectedObservation) -> SubjectBinding:
        if item.subject_reference in {"subject_ambiguous", "subject_missing"}:
            ...
            return SubjectBinding(
                ...
                subject_candidates=item.subject_candidates or ["current_pet"],
                confidence=0.9,
            )
