"""
=============================================================================
文件：src/vet_agent/followup_rag/retriever.py
作用：实现追问相关 RAG 的 PostgreSQL/pgvector 仓储与 LlamaIndex 检索适配。
范围：生产仓储只读取现有 knowledge_chunks 表中的已审核启用向量知识；
      LlamaIndex 仅用于标准 Node 封装和检索抽象，不创建第二套生产知识表。
说明：本文件是追问 RAG 的数据召回边界。无 embedding、无数据库、无合格
...
from .models import FollowupRagRetrievalResult
from .ports import FollowupRagKnowledgeRepository, FollowupRagRetriever


class PostgresFollowupRagKnowledgeRepository(FollowupRagKnowledgeRepository):
    """通过 SQLAlchemy 读取 knowledge_chunks 的追问 RAG 生产仓储。

    :return: 无返回值；本实现是追问 RAG 的 PostgreSQL/pgvector 主路径。
    """

    def __init__(self, database_url: str) -> None:
        """初始化 PostgreSQL 追问 RAG 知识仓储。
    ...
    def retrieve_by_embedding(
        ...
        *,
        ...
        :raises FollowupRagDependencyError: 数据库查询失败或 embedding 为空时抛出。
        """
        if not query_embedding:
            raise FollowupRagDependencyError(
                "followup RAG query embedding is empty",
        ...
        try:
            ...
            raise FollowupRagDependencyError(
                "followup RAG pgvector retrieval failed",
...
class LlamaIndexFollowupKnowledgeRetriever(FollowupRagRetriever):
    """将业务知识仓储结果封装为 LlamaIndex 节点的追问 RAG 检索器。
    ...
    def __init__(
        self,
        *,
        repository: FollowupRagKnowledgeRepository,
        embedding_client: EmbeddingClient,
    ) -> None:
        """初始化 LlamaIndex 追问 RAG 检索适配器。
    ...
    def retrieve(
        ...
        *,
        ...
        """执行追问 RAG 知识召回并封装 LlamaIndex 节点。
        ...
        :param allowed_chunk_types: 允许参与追问召回的知识 chunk 类型。
        :return: 返回检索结果与审计摘要。
        :raises FollowupRagDependencyError: embedding 或仓储召回不可用时抛出。
        """
        if not query.strip():
            raise FollowupRagDependencyError(
                "followup RAG retrieval query is empty",
                details={"reason": "empty_query"},
            )
        if not self.embedding_client.available:
            raise FollowupRagDependencyError(
                "followup RAG embedding client is unavailable",
        ...
        except FollowupRagDependencyError:
            raise
        except Exception as exc:
            raise FollowupRagDependencyError(
                "followup RAG retrieval failed",
                details={"error_type": type(exc).__name__},
            ) from exc
        if not hits:
            raise FollowupRagDependencyError(
    ...
    def _nodes_from_hits(self, hits: list[KnowledgeHit]) -> list[NodeWithScore]:
        ...
        for index, hit in enumerate(hits):
            ...
            evidence_id = str(metadata.get("chunk_id") or f"followup_hit_{index + 1}")
            metadata.update(
                {
                    "title": hit.title,
                    "source": hit.source,
    ...
    def _hits_from_nodes(
        ...
        """
        hit_by_id = {
            str(dict(hit.metadata or {}).get("chunk_id") or f"followup_hit_{index + 1}"): hit
            for index, hit in enumerate(hits)
        }
