...
class Settings:
    ...
    answer_rag_allowed_chunk_types: tuple[str, ...] = (
    ...
    clinical_safety_precondition_batch_size: int = 1
    clinical_safety_precondition_max_concurrency: int = 2
    clinical_safety_precondition_batch_timeout_seconds: float = 4.0
    clinical_safety_precondition_total_timeout_seconds: float = 6.0
    clinical_safety_precondition_min_confidence: float = 0.65
    ...
    def from_env(cls) -> "Settings":
        ...
        nltk_data_dir = Path(
            nltk_data_env.split(os.pathsep)[0].strip() or "/opt/vet-agent/nltk_data"
        )
        return cls(
            ...
            request_timeout_seconds=float(
                os.getenv(
                    "LITELLM_TIMEOUT_SECONDS", os.getenv("QWEN_TIMEOUT_SECONDS", "30")
                )
            ),
            data_dir=Path(os.getenv("VET_AGENT_DATA_DIR", ".data")),
            nltk_data_dir=nltk_data_dir,
            litellm_local_model_cost_map=_bool_env(
                "LITELLM_LOCAL_MODEL_COST_MAP", True
            ),
            seed_dir=Path(os.getenv("VET_AGENT_SEED_DIR", "assets/seeds")),
            clinical_safety_vector_min_score=float(
                os.getenv("CLINICAL_SAFETY_VECTOR_MIN_SCORE", "0.35")
            ),
            followup_rag_top_k=int(os.getenv("FOLLOWUP_RAG_TOP_K", "4")),
            followup_rag_vector_min_score=float(
                os.getenv("FOLLOWUP_RAG_VECTOR_MIN_SCORE", "0.35")
            ),
            answer_rag_top_k=int(os.getenv("ANSWER_RAG_TOP_K", "5")),
            answer_rag_vector_min_score=float(
                os.getenv("ANSWER_RAG_VECTOR_MIN_SCORE", "0.35")
            ),
            answer_rag_allowed_chunk_types=(
                _csv_env("ANSWER_RAG_ALLOWED_CHUNK_TYPES")
                or DEFAULT_ANSWER_RAG_ALLOWED_CHUNK_TYPES
            ),
            ...
            task_routing_max_task_count=int(
                os.getenv("TASK_ROUTING_MAX_TASK_COUNT", "5")
            ),
            ...
            task_routing_opa_rule_name=os.getenv(
                "TASK_ROUTING_OPA_RULE_NAME", "decision"
            ).strip(),
            task_routing_opa_auth_token=os.getenv("TASK_ROUTING_OPA_AUTH_TOKEN")
            or None,
            ...
            consultation_answerability_opa_auth_token=os.getenv(
                "CONSULTATION_ANSWERABILITY_OPA_AUTH_TOKEN"
            )
            or None,
            enable_llm_semantic_extraction=_bool_env(
                "ENABLE_LLM_SEMANTIC_EXTRACTION", True
            ),
            semantic_extraction_min_confidence=float(
                os.getenv("SEMANTIC_EXTRACTION_MIN_CONFIDENCE", "0.65")
            ),
            clinical_safety_precondition_batch_size=int(
                os.getenv("CLINICAL_SAFETY_PRECONDITION_BATCH_SIZE", "1")
            ),
            clinical_safety_precondition_max_concurrency=int(
                os.getenv("CLINICAL_SAFETY_PRECONDITION_MAX_CONCURRENCY", "2")
            ),
            clinical_safety_precondition_batch_timeout_seconds=float(
                os.getenv("CLINICAL_SAFETY_PRECONDITION_BATCH_TIMEOUT_SECONDS", "4")
            ),
            clinical_safety_precondition_total_timeout_seconds=float(
                os.getenv("CLINICAL_SAFETY_PRECONDITION_TOTAL_TIMEOUT_SECONDS", "6")
            ),
            clinical_safety_precondition_min_confidence=float(
                os.getenv("CLINICAL_SAFETY_PRECONDITION_MIN_CONFIDENCE", "0.65")
            ),
            consultation_max_followup_rounds=int(
                os.getenv("CONSULTATION_MAX_FOLLOWUP_ROUNDS", "2")
            ),
            ...
            memory_read_session_turn_limit=int(
                os.getenv("MEMORY_READ_SESSION_TURN_LIMIT", "20")
            ),
            memory_read_pet_episode_limit=int(
                os.getenv("MEMORY_READ_PET_EPISODE_LIMIT", "10")
            ),
            memory_read_semantic_limit=int(
                os.getenv("MEMORY_READ_SEMANTIC_LIMIT", "5")
            ),
            memory_read_allow_semantic_degraded=_bool_env(
            ...
            idempotency_processing_ttl_seconds=float(
                os.getenv("IDEMPOTENCY_PROCESSING_TTL_SECONDS", "300")
            ),
            qwen_max_concurrent_requests=int(
                os.getenv("QWEN_MAX_CONCURRENT_REQUESTS", "8")
            ),
            qwen_min_interval_seconds=float(
                os.getenv("QWEN_MIN_INTERVAL_SECONDS", "0")
            ),
            qwen_max_retries=int(os.getenv("QWEN_MAX_RETRIES", "2")),
            qwen_retry_base_delay_seconds=float(
                os.getenv("QWEN_RETRY_BASE_DELAY_SECONDS", "0.5")
            ),
            qwen_circuit_breaker_failure_threshold=int(
                os.getenv("QWEN_CIRCUIT_BREAKER_FAILURE_THRESHOLD", "5")
            ),
            qwen_circuit_breaker_cooldown_seconds=float(
                os.getenv("QWEN_CIRCUIT_BREAKER_COOLDOWN_SECONDS", "30")
            ),
            qwen_fallback_models=_csv_env("QWEN_FALLBACK_MODELS"),
            enable_memory_extraction=_bool_env("ENABLE_MEMORY_EXTRACTION", True),
            background_tasks_max_attempts=int(
                os.getenv("BACKGROUND_TASKS_MAX_ATTEMPTS", "5")
            ),
            background_tasks_worker_batch_size=int(
                os.getenv("BACKGROUND_TASKS_WORKER_BATCH_SIZE", "4")
            ),
            background_tasks_worker_lease_seconds=float(
                os.getenv("BACKGROUND_TASKS_WORKER_LEASE_SECONDS", "300")
            ),
            ...
            input_safety_policy_backend=os.getenv("INPUT_SAFETY_POLICY_BACKEND", "opa")
            .strip()
            .lower(),
            input_safety_policy_always_call=_bool_env(
                "INPUT_SAFETY_POLICY_ALWAYS_CALL", True
            ),
            input_safety_opa_base_url=os.getenv(
                "INPUT_SAFETY_OPA_BASE_URL", "http://opa:8181/v1"
            )
            .strip()
            .rstrip("/"),
            input_safety_opa_package_path=os.getenv(
                "INPUT_SAFETY_OPA_PACKAGE_PATH", "vet_agent.input_safety"
            ).strip(),
            input_safety_opa_rule_name=os.getenv(
                "INPUT_SAFETY_OPA_RULE_NAME", "decision"
            ).strip(),
            input_safety_opa_auth_token=os.getenv("INPUT_SAFETY_OPA_AUTH_TOKEN")
            or None,
            enable_output_safety=enable_output_safety,
            output_safety_mode=os.getenv(
                "OUTPUT_SAFETY_MODE", output_safety_mode_default
            )
            .strip()
            .lower(),
            output_safety_policy_backend=os.getenv(
                "OUTPUT_SAFETY_POLICY_BACKEND", "local"
            )
            .strip()
            .lower(),
            output_safety_policy_always_call=_bool_env(
                "OUTPUT_SAFETY_POLICY_ALWAYS_CALL", False
            ),
            ...
            output_safety_opa_rule_name=os.getenv(
            ...
            enable_output_safety_guardrails=_bool_env(
            ...
            clinical_safety_opa_rule_name=os.getenv(
            ...
            enable_input_safety_guardrails=_bool_env(
            ...
            input_safety_prompt_injection_threshold=float(
                os.getenv("INPUT_SAFETY_PROMPT_INJECTION_THRESHOLD", "0.8")
            ),
