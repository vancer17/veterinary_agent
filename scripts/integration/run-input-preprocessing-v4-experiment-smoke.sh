#!/usr/bin/env bash
# =============================================================================
# 文件: scripts/integration/run-input-preprocessing-v4-experiment-smoke.sh
# 作用: 通过远程 LiteLLM 执行第四轮 input-preprocessing V4 扁平抽取 shadow 实验。
# 范围: 覆盖 quote 锚定、共享断言展开、主体角色、canonical 候选引用、
#       口语脏数据、多轮上下文、负例 gate、持久化异步隔离和 clinical safety
#       report-only 边界。
...
litellm_local_port="${INPUT_PREPROCESSING_LITELLM_TUNNEL_PORT:-54008}"
...
if [[ -n "${INPUT_PREPROCESSING_V4_EXPERIMENTS:-}" ]]; then
    IFS=',' read -ra experiment_ids <<<"${INPUT_PREPROCESSING_V4_EXPERIMENTS}"
