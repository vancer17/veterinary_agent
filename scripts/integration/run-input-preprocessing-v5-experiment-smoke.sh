#!/usr/bin/env bash
# =============================================================================
# 文件: scripts/integration/run-input-preprocessing-v5-experiment-smoke.sh
# 作用: 通过远程 LiteLLM 执行第五轮 input-preprocessing V5 薄声明 shadow 实验。
# 范围: 覆盖独立意图路由、ThinUserClaim、quote 治理、per-claim graph、
#       按需主体/参与者/时间/度量富化、target quote canonical 链接、
#       负例 gate、异步隔离和 clinical safety report-only 边界。
...
litellm_local_port="${INPUT_PREPROCESSING_LITELLM_TUNNEL_PORT:-54009}"
...
if [[ -n "${INPUT_PREPROCESSING_V5_EXPERIMENTS:-}" ]]; then
    IFS=',' read -ra experiment_ids <<<"${INPUT_PREPROCESSING_V5_EXPERIMENTS}"
