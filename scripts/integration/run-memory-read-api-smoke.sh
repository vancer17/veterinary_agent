#!/usr/bin/env bash
# =============================================================================
# 文件: scripts/integration/run-memory-read-api-smoke.sh
# 作用: 运行接入真实 PostgreSQL、Mem0、LiteLLM 与 OPA 的记忆读取 API 集成测试。
# 范围: 覆盖 Agent API 记忆读取主路径、Mem0 语义投影召回、PostgreSQL 权威记忆读取与范围审计。
# 说明: 本脚本只读取调用方显式注入的环境变量，不写入或推导真实密钥；默认通过 SSH 隧道访问远程开发依赖。
# =============================================================================
...
required_vars=(
    ...
    "EXTERNAL_API_TEST_MEM0_API_KEY"
)
...
for var_name in "${required_vars[@]}"; do
    if [[ -z "${!var_name:-}" ]]; then
        echo "缺少记忆读取外部 API 集成测试环境变量: ${var_name}" >&2
        exit 1
    fi
done
...
if [[ -n "${EXTERNAL_API_TEST_SSH_HOST:-}" ]]; then
    ...
    start_ssh_tunnel() {
        ...
        # 真实服务公网入口可能受网关路径和网络波动影响；
        # 隧道模式通过远程主机本机端口访问依赖，便于区分业务链路问题和公网代理问题。
        ssh \
            -i "${ssh_key}" \
            -p "${ssh_port}" \
...
PY
    ...
    case "${tunnel_http_services}" in
        true)
            ...
            mem0_local_port="${EXTERNAL_API_TEST_MEM0_TUNNEL_LOCAL_PORT:-58001}"
...
export RUN_MEMORY_READ_API_EXTERNAL_TEST=true

pytest_args=("$@")
case "${pytest_args[0]:-}" in
    ""|-*)
        pytest_args=("tests/integration/test_memory_read_api_external.py" "${pytest_args[@]}")
        ;;
esac
